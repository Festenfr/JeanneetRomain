...
export default class Scene {
    ...
    tick() {
        ...
        gsap.set(this.plane.rotation, {
            y: -ScrollHelper.easeSlow / window.innerHeight,
            z: ScrollHelper.easeSlow / 4 / window.innerHeight
        })
        gsap.set(this.camera.rotation, {
            x: ScrollHelper.easeSlow / 2 / window.innerHeight
        })
        gsap.set(this.uniforms.alpha, {
            value:
                (-ScrollHelper.ease / 6 + window.innerHeight) /
                (window.innerHeight + ScrollHelper.ease)
        })
        gsap.set(this.uniforms.progress, {
            value: (ScrollHelper.easeSlow / window.innerHeight) * 1.5
        })
    }
    calcImageAspect() {
        this.imageAspect =
